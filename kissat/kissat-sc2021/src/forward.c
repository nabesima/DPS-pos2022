...
find_forward_subsumption_candidates (kissat * solver, references * candidates)
{
  ...
  for (all_clauses (c))
	...
	continue;
      ...
      // added by nabesima for DPS
      if (solver->dps_ticks++ > 10000)
      	(void)dps_check_period(solver, "forward1");
  ...
  while (p != end)
      ...
      else
	{
	  ...
	  subsume = true;
          ...
          // added by nabesima for DPS
          if (solver->dps_ticks++ > 10000)
            (void)dps_check_period(solver, "forward2");
...
forward_subsume_all_clauses (kissat * solver)
{
...

  {
	...
	// added by nabesima for DPS
        if (solver->dps_ticks++ > 10000)
      	  (void)dps_check_period(solver, "forward3");
  ...
  for (reference * q = BEGIN_STACK (candidates); q != end_of_candidates; q++)
	...
	continue;
      ...
      // added by nabesima for DPS
      if (solver->dps_ticks++ > 10000)
    	(void)dps_check_period(solver, "forward4");
