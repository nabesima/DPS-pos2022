...
// added by nabesima for DPS
static bool
dps_apply_imported_clauses(kissat *solver) {
  void *wrapper = solver->wrapper;
  if (!wrapper) 
    return true;
  if (solver->level > 0 && !DPS_shouldApplyImportedClauses(wrapper)) 
    return true;
  return DPS_kissat_applyImportedClauses(wrapper);
}
...
kissat_search (kissat * solver)
{
  ...
  while (!res)
    {	    
      // added by nabesima for DPS
      if (!dps_check_period (solver, "search"))
        break;
      if (!dps_apply_imported_clauses (solver)) {
        res = 20;
	break;
      }
