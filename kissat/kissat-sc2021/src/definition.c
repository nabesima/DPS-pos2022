			       ...
			       size_t msize, const unsigned *mlits)
{
  ...
  if (msize)
      ...
      for (const unsigned *p = mlits; p != end; p++)
	{
...
#if !defined(NDEBUG) && !defined(NCHECKS) // modified by nabesima to suppress unused variable warning
      ...
      assert (offset + msize + 1 == SIZE_STACK (*added));
      CHECK_AND_ADD_LITS (msize + 1, lits);
      ADD_LITS_TO_PROOF (msize + 1, lits);
    }
  else
    {
