		   ...
		   uint64_t * reduced_ptr, unsigned *units)
{
  ...
  // added by nabesima for DPS
  solver->dps_ticks += src_ticks;
  (void)dps_check_period(solver, "transitive1");
  ...
  for (watch * p = begin_src; p != end_src; p++)
      ...
      while (!transitive && !failed && propagate != END_ARRAY (solver->trail))
	{
      ...
      // added by nabesima for DPS
      solver->dps_ticks += inner_ticks;
      (void)dps_check_period(solver, "transitive2");
...
static inline bool
less_stable_transitive (kissat * solver,
...
less_focused_transitive (kissat * solver,
...
static void
sort_focused_transitive (kissat * solver, unsigneds * probes)
{
  const flags *const flags = solver->flags;
...
static void
sort_transitive (kissat * solver, unsigneds * probes)
{
  if (solver->stable)
...
static void
schedule_transitive (kissat * solver, unsigneds * probes)
{
  ...
  while (!terminate && !EMPTY_STACK (probes))
      ...
      for (unsigned sign = 0; !terminate && sign < 2; sign++)
	{
	  ...
	  if (solver->inconsistent)
	    ...
	    terminate = true;
	  else if (solver->should_be_terminated)  // added by nabesima for DPS
	    terminate = true;
	}
    }
