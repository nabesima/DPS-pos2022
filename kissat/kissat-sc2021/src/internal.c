...
//#ifndef NDEBUG
#if !defined(NDEBUG) && !defined(NCHECKS) // modified by nabesima 
  ...
  solver->thn = 0;
  solver->solver_state = ' ';
  solver->should_be_terminated = false;
  solver->rand_pick_until_1st_conf = false;
  solver->dps_ticks = 0;
  INIT_STACK (solver->export_clause);
