...
kissat_next_decision_variable (kissat * solver)
{
  ...
  if (solver->rand_pick_until_1st_conf && solver->statistics.conflicts == 0) {
    unsigned idx = kissat_pick_random(&solver->random, 0, kissat_size_heap(&solver->scores));
    if (kissat_heap_contains(&solver->scores, idx) && ACTIVE (idx)) {
      res = kissat_get_heap(&solver->scores, idx);    
      if (!solver->values[LIT (res)]) {
        LOG ("next random decision %s", LOGVAR (res));
        return res;
      }
    }
  }
